...
type packing struct {
	...
	unpackall func(uint64, []uint64)
	pack      func([]uint64) uint64
}

var selector [16]packing = [16]packing{
	packing{240, 0, unpack240, unpack240all, pack240},
	packing{120, 0, unpack120, unpack120all, pack120},
	packing{60, 1, unpack60, unpack60all, pack60},
	packing{30, 2, unpack30, unpack30all, pack30},
	packing{20, 3, unpack20, unpack20all, pack20},
	packing{15, 4, unpack15, unpack15all, pack15},
	packing{12, 5, unpack12, unpack12all, pack12},
	packing{10, 6, unpack10, unpack10all, pack10},
	packing{8, 7, unpack8, unpack8all, pack8},
	packing{7, 8, unpack7, unpack7all, pack7},
	packing{6, 10, unpack6, unpack6all, pack6},
	packing{5, 12, unpack5, unpack5all, pack5},
	packing{4, 15, unpack4, unpack4all, pack4},
	packing{3, 20, unpack3, unpack3all, pack3},
	packing{2, 30, unpack2, unpack2all, pack2},
	packing{1, 60, unpack1, unpack1all, pack1},
}
...
func DecodeAll(dst, src []uint64) (value int, err error) {
	...
	for _, v := range src {
		sel := (v >> 60) & 0xf
		selector[sel].unpackall(v, dst[j:])
		j += selector[sel].n
	}
	return j, nil
}
...
func unpack240all(v uint64, dst []uint64) {
	for i := range dst[:240] {
		dst[i] = 1
	}
}

func unpack120all(v uint64, dst []uint64) {
	for i := range dst[:120] {
		dst[i] = 1
	}
}

func unpack60all(v uint64, dst []uint64) {
	dst[0] = v & 1
	dst[1] = (v >> 1) & 1
	dst[2] = (v >> 2) & 1
	dst[3] = (v >> 3) & 1
...
func unpack30all(v uint64, dst []uint64) {
	dst[0] = v & 3
	dst[1] = (v >> 2) & 3
	dst[2] = (v >> 4) & 3
	dst[3] = (v >> 6) & 3
...
func unpack20all(v uint64, dst []uint64) {
	dst[0] = v & 7
	dst[1] = (v >> 3) & 7
	dst[2] = (v >> 6) & 7
	dst[3] = (v >> 9) & 7
...
func unpack15all(v uint64, dst []uint64) {
	dst[0] = v & 15
	dst[1] = (v >> 4) & 15
	dst[2] = (v >> 8) & 15
	dst[3] = (v >> 12) & 15
...
func unpack12all(v uint64, dst []uint64) {
	dst[0] = v & 31
	dst[1] = (v >> 5) & 31
	dst[2] = (v >> 10) & 31
	dst[3] = (v >> 15) & 31
...
func unpack10all(v uint64, dst []uint64) {
	dst[0] = v & 63
	dst[1] = (v >> 6) & 63
	dst[2] = (v >> 12) & 63
	dst[3] = (v >> 18) & 63
...
func unpack8all(v uint64, dst []uint64) {
	dst[0] = v & 127
	dst[1] = (v >> 7) & 127
	dst[2] = (v >> 14) & 127
	dst[3] = (v >> 21) & 127
...
func unpack7all(v uint64, dst []uint64) {
	dst[0] = v & 255
	dst[1] = (v >> 8) & 255
	dst[2] = (v >> 16) & 255
	dst[3] = (v >> 24) & 255
...
func unpack6all(v uint64, dst []uint64) {
	dst[0] = v & 1023
	dst[1] = (v >> 10) & 1023
	dst[2] = (v >> 20) & 1023
	dst[3] = (v >> 30) & 1023
...
func unpack5all(v uint64, dst []uint64) {
	dst[0] = v & 4095
	dst[1] = (v >> 12) & 4095
	dst[2] = (v >> 24) & 4095
	dst[3] = (v >> 36) & 4095
...
func unpack4all(v uint64, dst []uint64) {
	dst[0] = v & 32767
	dst[1] = (v >> 15) & 32767
	dst[2] = (v >> 30) & 32767
	dst[3] = (v >> 45) & 32767
...
func unpack3all(v uint64, dst []uint64) {
	dst[0] = v & 1048575
	dst[1] = (v >> 20) & 1048575
	dst[2] = (v >> 40) & 1048575
}

func unpack2all(v uint64, dst []uint64) {
	dst[0] = v & 1073741823
	dst[1] = (v >> 30) & 1073741823
}

func unpack1all(v uint64, dst []uint64) {
	dst[0] = v & 1152921504606846975
}
